...
async function scrapeHolepunch(page, baseUrl) {
  ...
  for (const jobItem of jobListingItems) {
    const jobTitle = await jobItem.$eval('.sc-6exb5d-1.fmfYYf', element => element.textContent.trim());

    const jobUrl = await jobItem.$eval('.sc-6exb5d-1.fmfYYf', element => element.getAttribute('href'));
